...
echo -e "${PINK}${logo}${NC}"
echo -e "${PURPLE}Dotfiles Auto Config!${NC}"
...
clone_repository_with_progress "https://github.com/ByChanderZap/dotfiles.git" "Dotfiles" 20
cd Dotfiles || exit
  ...
  "kitty")
    if [ "$os_choice" = "mac" ]; then
      if ! command -v kitty &>/dev/null; then
        install_terminal_with_progress "Kitty" "brew install --cask kitty" "mkdir -p ~/.config/kitty && cp -r KittyConfigs/* ~/.config/kitty"
      else
        mkdir -p ~/.config/kitty && cp -r KittyConfigs/* ~/.config/kitty
...
"nushell")
  ...
  if [[ "$OSTYPE" == "darwin"* ]]; then
    ...
    update_or_replace "NushellConfigs/env.nu" "/home/linuxbrew/.linuxbrew/bin" "    | prepend '/opt/homebrew/bin'"
    run_command "cp -rf NushellConfigs/* ~/Library/Application\ Support/nushell/"
  else
    mkdir -p ~/.config/nushell
    run_command "cp -rf NushellConfigs/* ~/.config/nushell/"
  fi
...
"fish")
  ...
  run_command "cp -rf FishConfigs/fish ~/.config"
  ;;
"zsh")
  ...
  run_command "cp -rf FishConfigs/.zshrc ~/"
  run_command "cp -rf FishConfigs/.p10k.zsh ~/"
  run_command "cp -rf FishConfigs/.oh-my-zsh ~/"
...
# Ask if window manager
echo -e "${YELLOW}Step 4: Choose and Install Window Manager${NC}"
wm_choice=$(select_option "Would you like zellij as window manager? " "yes" "no")
...
"yes")
  ...
  run_command "cp -r ZellijConfigs/zellij/* ~/.config/zellij/"
  ...
  if [[ "$shell_choice" == "zsh" ]]; then
    ...
    if [[ "$os_type" == "Darwin" ]]; then
      update_or_replace "ZellijConfigs/config.nu" '"tmux"' 'let MULTIPLEXER = "zellij"'
      update_or_replace "ZellijConfigs/config.nu" '"TMUX"' 'let MULTIPLEXER_ENV_PREFIX = "ZELLIJ"'
      run_command "cp -rf ZellijConfigs/* ~/Library/Application\ Support/nushell/"
    else
...
"no")
  ...
  if [ -f ~/.config/fish/config.fish ] || [ -d ~/Library/Application\ Support/nushell ]; then
    if [[ "$os_type" == "Darwin" ]]; then
      update_or_replace "NushellConfigs/config.nu" "run-external \$MULTIPLEXER" "true"
    else
...
echo -e "${YELLOW}Step 5:NVIM install${NC}"
  ...
  run_command "cp -r NvimConfigs/nvim/* ~/.config/nvim/"
